...
namespace Go
{
    public partial class GoSearch : HomoryResourcePage
    {
        ...
        protected void search_go_OnServerClick(object sender, EventArgs e)
        {
            ...
            period.DataSource = source.Where(s => s.GradeId != null).Select(s => s.GradeId).Distinct().ToList().Join(HomoryContext.Value.Catalog.Where(c => c.State < State.审核), a => a, b => b.Id, (a, b) => b).Select(o => o.Type).Distinct().ToList();
            period.DataBind();
