...
namespace Go
{
        ...
        protected User TargetUser
        {
            get
            {
                if (_target == null)
                {
                    ...
                    _target = HomoryContext.Value.User.Single(o => o.Id == id);
                }
                return _target;
            }
        }
    ...
    TargetUser.Resource.Where(o => o.State == State.启用)
        ...
        {
            var user = TargetUser;
            result1.DataSource = HomoryContext.Value.Resource.Where(o => o.Type == ResourceType.视频 && o.UserId == user.Id && o.State == State.启用).ToList();
        }

        protected void result2_NeedDataSource(object sender, RadListViewNeedDataSourceEventArgs e)
        {
            var user = TargetUser;
            result2.DataSource = HomoryContext.Value.Resource.Where(o => o.Type == ResourceType.文章 && o.UserId == user.Id && o.State == State.启用).ToList();
        }

        protected void result3_NeedDataSource(object sender, RadListViewNeedDataSourceEventArgs e)
        {
            var user = TargetUser;
            result3.DataSource = HomoryContext.Value.Resource.Where(o => o.Type == ResourceType.课件 && o.UserId == user.Id && o.State == State.启用).ToList();
        }

        protected void result4_NeedDataSource(object sender, RadListViewNeedDataSourceEventArgs e)
        {
            var user = TargetUser;
            result4.DataSource = HomoryContext.Value.Resource.Where(o => o.Type == ResourceType.试卷 && o.UserId == user.Id && o.State == State.启用).ToList();
        }
    }
}
