...
TargetUser.Resource.Where(o => o.State == State.启用 && o.Type == rt)
	...
	.ToList();
		...
		protected void mnbtn_ServerClick(object sender, EventArgs e)
		{
			...
			{
				mnp.ResponseScripts.Add(string.Format("window.alert('过程记录不得少于50字，当前{0}字！');", 50 - mn1.InnerText.Length));
				return;
			}
				...
				if (HomoryContext.Value.AssessTable.Count(o => o.CourseId == 课程 && o.GradeId == 年级 && o.State == State.启用) > 0)
				{
					...
					for (int k = 0; k < list.Count; k++)
					{
						if (____c > 0)
						{
							...
							list[k].All = string.Format("（{0}/{1}分）", ras.Average(p => p[k]).ToString("F0"), list[k].Score.ToString("F0"));
						}
						else
						{
							list[k].All = string.Format("（{0}/{1}分）", 0, list[k].Score.ToString("F0"));
						}
					}
					sss.InnerText = string.Format("共{0}次评估，得分{1}分", ras.Count, total.ToString("F0"));
