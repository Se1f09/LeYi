...
namespace Go
{
	public partial class GoQueryStudent : HomoryCorePageWithGrid
	{
        ...
        protected void grid_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            if (CurrentRights.Contains(HomoryCoreConstant.RightGlobal))
            {
                grid.DataSource = HomoryContext.Value.ViewQueryStudent.ToList();
            }
            else
            {
                var campus = CurrentCampus.Name;
                grid.DataSource = HomoryContext.Value.ViewQueryStudent.Where(o => o.学校 == campus).ToList();
            }
        }
