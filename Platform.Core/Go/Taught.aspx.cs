...
// ReSharper restore PossibleNullReferenceException
        ...
        protected void view_OnNeedDataSource(object sender, RadListViewNeedDataSourceEventArgs e)
        {
            ...
            view.DataSource = list == null ? null : list.Where(o => o.Account.Contains(stext) || o.RealName.Contains(stext) || o.Phone.Contains(stext) || o.IDCard.Contains(stext)).ToList();
        }
        ...
        public bool CourseUntaught(Entities db, Guid classId, Guid courseId)
        {
            try
            {
                db.Taught.Where(o => o.DepartmentId == classId && o.CourseId == courseId && o.State == State.启用).Update(o => new Taught { State = State.删除 });
                db.SaveChanges();
