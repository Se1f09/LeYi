	...
	public partial class GoSignOn : HomoryPage
	{
				...
				if (IsSsoOnline(out user))
				{
					...
					else
					{
						...
						else
						{
							if (query.IndexOf('&') <= 0)
							{
								url = string.Format("{0}?OnlineId={1}", path, user.UserOnline.Single().Id);
							}
							else
							{
								var index = query.IndexOf('&');
								url = string.Format("{0}?OnlineId={1}{2}", path, user.UserOnline.Single().Id, query.Substring(index));
							}
						}
		...
		protected void buttonSign_OnClick(object sender, EventArgs e)
		{
			...
			if (output.Ok)
			{
				...
				{
					Response.Redirect(output.Data.RedirectUrl, false);
					return;
				}
			}
